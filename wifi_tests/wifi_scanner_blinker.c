...
bool led_timer_callback(struct repeating_timer *unused_t) {
    led_state = !led_state;
        ...
        if (absolute_time_diff_us(get_absolute_time(), scan_time) < 0) {
            if (!scan_in_progress) {
                ...
                if (err == 0) {
                    ...
                    // Start the LED blinking timer (100ms interval)
                ...
                cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, 0);  // dumbly turn off the LED
                print_system_info(&n_iteration);
            }
        }
    }
